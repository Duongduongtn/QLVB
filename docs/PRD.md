...
- **Ưu tiên**: **Must**
- **Trạng thái**: ⚠️ Partial (27/06/2026) — **backend + FE XONG gồm trọn vòng đời** (PDF-only): tải PDF→metadata→hồ sơ ký lọc đơn vị→preview chèn mộc (PyMuPDF, auto góc dưới phải trang cuối)→giáp lai/ký nháy→**xác nhận chống nhầm hiện tên đơn vị**→cấp số atomic / dùng số có sẵn→tải `_CHUA_KY_SO`→**upload bản đã ký số (D1.12, check số trong tên file token-match) → "Đã phát hành"**; **Huỷ/Thu hồi** (giữ số không tái dùng, **thu hồi CV đã phát hành chỉ Quản lý**). Chống nhầm mộc enforce server; khoá row mọi thao tác đổi trạng thái. **Defer:** convert Word (LibreOffice-worker), kéo-thả vị trí mộc (D2-D, đang auto), verify chữ ký số PAdES bản upload (E1.5), watermark khi tải (D1.11/H2).
- **Steps to Complete**:
