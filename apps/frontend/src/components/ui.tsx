...
/* Primitives dùng chung — port từ ui-demo/src/components/ui.tsx. */
...
/* ---------- Filter menu (dropdown chức năng, style như chip .filter-select) ---------- */
            ...
            {options.map((o) => (
              <button
                ...
                style={{
                  ...
                  color: o.value === value ? 'var(--ink)' : 'var(--ink-body)',
                }}
              >
                {o.label}
              </button>
...
export function InfoRow({ label, children }: { label: string; children: ReactNode }) {
  return (
    ...
    >
      <span style={{ width: 116, flexShrink: 0, fontSize: '0.8rem', color: 'var(--ink-muted)' }}>
        {label}
      </span>
      <span style={{ flex: 1, minWidth: 0, fontSize: '0.875rem', color: 'var(--ink)' }}>
        {children}
      </span>
    </div>
  );
      ...
      {items.map((it, i) => (
        <div key={i} className="flex" style={{ gap: 12 }}>
          <div className="flex flex-col items-center" style={{ width: 12 }}>
            ...
            />
            {i < items.length - 1 && (
              <span style={{ width: 1, flex: 1, background: 'var(--rule)' }} />
            )}
          </div>
